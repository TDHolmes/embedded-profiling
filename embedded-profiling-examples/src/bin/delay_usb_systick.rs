...
fn main() -> ! {
    ...
    let systick_profiler = cortex_m::singleton!(: ep_systick::SysTickProfiler::<CORE_FREQ> =
        ep_systick::SysTickProfiler::new(core.SYST, CORE_FREQ))
    .unwrap();
    unsafe {
        ep::set_profiler(systick_profiler).unwrap();
    }
