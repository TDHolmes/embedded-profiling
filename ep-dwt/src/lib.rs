//! `Monotonic` implementation based on `DWT`
...
/// DWT trace unit implementing [`EmbeddedProfiler`](EmbeddedProfiler).
///
...
impl<const FREQ: u32> DwtProfiler<FREQ> {
    /// Enable the DWT and provide a new `Monotonic` based on `DWT`.
    ///
